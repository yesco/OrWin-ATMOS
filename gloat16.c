...
gloat16 gmul(gloat16 a, gloat16 b) {
  ...
  uint8_t exp_a = ca.bytes.meta & 0x3F;
  uint8_t exp_b = cb.bytes.meta & 0x3F;
  int16_t true_exp = (int16_t)(exp_a - 32) + (int16_t)(exp_b - 32);
  uint16_t frac_sum = (uint16_t)ca.bytes.fraction + cb.bytes.fraction;
  if (frac_sum >= 256) {
    true_exp++;
  }
...
gloat16 gdiv(gloat16 num, gloat16 den) {
  ...
  uint8_t exp_num = cnum.bytes.meta & 0x3F;
  uint8_t exp_den = cden.bytes.meta & 0x3F;
  int16_t true_exp = (int16_t)(exp_num - 32) - (int16_t)(exp_den - 32);
  int16_t frac_diff = (int16_t)cnum.bytes.fraction - cden.bytes.fraction;
  if (frac_diff < 0) {
    true_exp--;
    frac_diff += 256;
  }
...
gloat16 gadd(gloat16 a, gloat16 b) {
  ...
  uint8_t exp_a = ca.bytes.meta & 0x3F;
  uint8_t exp_b = cb.bytes.meta & 0x3F;
  int16_t delta_exp = (int16_t)exp_a - exp_b;
  int16_t delta_frac = (int16_t)ca.bytes.fraction - cb.bytes.fraction;
  int16_t delta = (delta_exp * 256) + delta_frac;
  if (delta >= 524) {
    return ca.raw;
  }
  if (delta == 0) {
    gloat_cast res = ca;
    uint16_t f_sum = (uint16_t)res.bytes.fraction + 77;
    if (f_sum >= 256) {
  ...
  gloat_cast final_res = ca;
  uint16_t final_frac_sum = (uint16_t)final_res.bytes.fraction + displacement_s;
...
gloat16 itog(int16_t val) {
  ...
  uint8_t digit_idx = (val > 0 && val <= 9) ? (val - 1) : 0;
  uint16_t frac16 = log_thresholds[digit_idx];
...
int16_t gtoi(gloat16 a) {
  gloat_cast ca;
  ca.raw = a;
  int8_t true_exp = (ca.bytes.meta & 0x3F) - 32;
  if (true_exp < 0) return 0;
  uint16_t full_frac = (uint16_t)ca.bytes.fraction * 256;
  int16_t digit = 1;
  for (int i = 8; i >= 0; i--) {
...
gloat16 atog(const char* str) {
  ...
  uint8_t digit_idx = (digit > 0 && digit <= 9) ? (digit - 1) : 0;
  uint32_t base_frac = log_thresholds[digit_idx];
  uint32_t next_frac = (digit < 9) ? log_thresholds[digit_idx + 1] : 65536;
  uint32_t gap = next_frac - base_frac;
  uint32_t interpolation = (dec1 * 10 + dec2) * gap / 100;
  uint16_t final_frac16 = (uint16_t)(base_frac + interpolation);
...
void gtoa(gloat16 a, char* buf) {
  ...
  uint16_t base_frac = log_thresholds[digit - 1];
  uint16_t next_frac = (digit < 9) ? log_thresholds[digit] : 65535;
  uint32_t rem = full_frac - base_frac;
  uint32_t gap = next_frac - base_frac;
  uint32_t interp = 0;
  if (gap > 0) {
    interp = (rem * 100) / gap;
...
int main(void) {
  char buf[32];
  gloat16 n1 = atog("3.14e6");
  gtoa(n1, buf);
  printf("atog/gtoa Test 1 (3.14e6): %s\n", buf);
  gloat16 n2 = atog("2.00");
  gloat16 n3 = gmul(n1, n2);
  gtoa(n3, buf);
  printf("gmul Test (3.14e6 * 2.00): %s\n", buf);
  gloat16 n4 = gdiv(n3, n2);
  gtoa(n4, buf);
  printf("gdiv Test (Result / 2.00): %s\n", buf);
  gloat16 n5 = atog("1.00e3");
  gloat16 n6 = atog("5.00e2");
  gloat16 n7 = gadd(n5, n6);
  gtoa(n7, buf);
  printf("gadd Test (1.00e3 + 5.00e2): %s\n", buf);
  gloat16 n8 = gsub(n7, n6);
  gtoa(n8, buf);
